...
### ⚠️ `LineAmountTypes` — tax basis of `LineAmount` (affects all transactional facts)
Xero's line `LineAmount` (staged as `line_amount`) has **no single meaning** — its tax basis is set by the document header's `LineAmountTypes`:
- `Exclusive` / `NoTax` → `line_amount` is **net** (tax-exclusive)
- `Inclusive` → `line_amount` **includes tax**

Summing raw `line_amount` across a mix overstates the `Inclusive` documents by their tax. Staging stores the raw value faithfully (correct for staging); the **ODS `_1` layer normalizes to net** (`Inclusive → line_amount − tax_amount`) so the measure is consistent and ties to header `sub_total`. This applies to invoices, credit notes, bank transactions, purchase orders, and quotes — each header carries its own `line_amount_types`. Full detail + the reconciliation assertion that enforces it: see "Xero tax basis" in `docs/DWH_ARCHITECTURE.md`.

### ⚠️ `journals` (GL) lines balance on `NetAmount`, NOT `GrossAmount` (verified 2026-07-09)
Each `JournalLine` in `staging_xero.journal_lines` carries three amount fields: `net_amount`, `gross_amount`, `tax_amount`. **Only `net_amount` sums to zero per journal** (double-entry balance) — verified 100% (899/899) across all 3 tenants with journals data, across every `source_type`. `gross_amount` includes tax on the originating expense/revenue line only (the tax posts to its own control-account line), so summing it never balances when a transaction has tax (only 54% "balanced" using gross). **Use `net_amount` as the GL posting measure** when building `fact_general_ledger` — this is the GL-equivalent of the `LineAmountTypes` issue above. 23 journal headers (`source_type IS NULL`) have zero lines — likely opening-balance/system journals, not a data defect.

### `organisations.FinancialYearEndDay`/`FinancialYearEndMonth` — needed for Trial Balance reconciliation (added 2026-07-24)
Xero's `Organisation` payload carries `FinancialYearEndDay` (int, day-of-month) and `FinancialYearEndMonth` (int, 1-12), staged as `financial_year_end_day`/`financial_year_end_month`. **Tenants have mixed fiscal years** — confirmed 2 of 7 use a June 30 year-end, the rest Dec 31 — so any YTD-style reconciliation against Xero's own reports (Trial Balance, P&L) must compute each tenant's fiscal year start from these fields, never assume calendar year. See `recon_trial_balance` in `docs/DWH_ARCHITECTURE.md` for the reconciliation this enables.

### `bank_transfers` / `overpayments` — restored 2026-07-24 to close GL-reconciliation gaps
Both were removed 2026-07-07 under the bucket-driven parser policy (bronze was empty at the time) and restored once `journals.source_type IN ('TRANSFER', 'APOVERPAYMENT', 'AROVERPAYMENT')` postings were traced back to them — see `docs/DWH_ARCHITECTURE.md` for the full mapping exercise. Both payloads were re-verified against live data before restoring.

**`bank_transfers`** (`/BankTransfers`) — money moved between two of the org's *own* bank accounts. Header only, no lines. `Reference` is free text, but some orgs enter a purely numeric value — Xero's JSON serializes those unquoted, so the raw payload value can be a string or a number depending on tenant. **Cast to `str()` explicitly in the parser** rather than trusting the raw type; see the schema-race note below for why the Python-level cast alone isn't sufficient. `Status`, `FromIsReconciled`/`ToIsReconciled` weren't captured by the original 2026-06 parser — added on restore per the staging-purity rule (capture every available scalar). ⚠ Every transfer also creates a matching SPEND + RECEIVE row in `bank_transactions` (linked via `FromBankTransactionID`/`ToBankTransactionID`) — never sum `bank_transfers.amount` alongside `bank_transactions` in the same total, that double-counts the same cash movement.

**`overpayments`** (`/Overpayments`) — money received/paid beyond what was owed, before allocation to a future invoice. Header + `LineItems[]` (added on restore — the original 2026-06 parser only counted lines, it didn't unpack them; the account-level grain a GL reconciliation needs lives in the lines, not the header total). `Type` distinguishes AR (`RECEIVE-OVERPAYMENT`) from AP (`SPEND-OVERPAYMENT`) — confirmed these map 1:1 to `journals.source_type` `AROVERPAYMENT`/`APOVERPAYMENT` respectively. Carries `LineAmountTypes` just like invoices/credit notes — the same net-of-tax normalization rule applies (see "Xero tax basis" below); `ods_xero_1.fact_overpayment_line` normalizes and is guarded by `assert_fact_overpayment_line_reconciles`.

**⚠ Schema-autodetect trap, worth internalizing for any future parser restoration.** Restoring these two hit the same class of bug as the `manual_journal_lines.account_code` and `organisations.registration_number` incidents, but with a twist: even an explicit `str()` cast in the Python parser did **not** prevent BigQuery from inferring `reference` as `INTEGER` on first table creation, because BigQuery's `autodetect=True` examines the actual string *content* (not just its JSON type) — an all-digits string like `"7052271"` gets typed as a number regardless of it being quoted. **The only reliable fix is pre-creating the target table with an explicit schema before the first backfill**, sidestepping autodetect entirely; the `str()` cast is defense-in-depth, not the actual fix. Also found (and fixed in `etl/common/bq_writer.py`) a genuine concurrency bug this surfaced: for a table that doesn't exist yet, the pre-lock schema read let two tenants' first-ever writes race to autodetect differently-typed schemas — the schema read now happens inside the per-table lock.

### Reports API (`report_*` endpoints) — a fundamentally different shape (added 2026-07-24)
Six report kinds are synced: `report_balance_sheet`, `report_bank_summary`, `report_budget_summary`, `report_executive_summary`, `report_profit_and_loss`, `report_trial_balance`. (The old silver layer also handled `aged_receivables_by_contact`/`aged_payables_by_contact` — confirmed via full bucket listing across all 7 tenants that neither exists in the current sync, under any name.)

**Shape**: `{"Reports": [{ReportID, ReportType, ReportName, ReportTitles, Fields: [], Rows: [...]}]}`. `Rows[]` is a **tree**, not a flat array — `RowType` is `Header` (defines column labels by cell position), `Section` (nests the real data in its own `Rows[]`), or `Row`/`SummaryRow` (the data itself). Each cell is `{Value, Attributes: [{Id, Value}]}` — `Attributes` links a cell back to an entity, e.g. `{"Id": "account", "Value": "<account-guid>"}`. **Not every `account` attribute is a real UUID** — some are Xero synthetic group sentinels (observed: `"FXGROUPID"` on a rolled-up FX gain/loss line). A reconciliation join must filter for UUID-shaped `attribute_value`s.

**`ReportID`/`ReportType` are fixed per report KIND** (e.g. `"ProfitAndLoss"`), not unique per run — cannot serve as a snapshot identifier. Report params (`fromDate`/`toDate` or a single `date`) live **only in the `.meta.json` sidecar** as `x-report-params`/`x-report-from`/`x-report-to` — the payload's own `Fields` array is empty on every kind observed. `x-run-id` (a fresh UUID per sync) is the only reliable per-snapshot identifier.

**⚠️ These are snapshots, not entities — staging must never upsert-collapse them.** Every other Xero parser keys on `(tenant_id, record_id)` = one current row per entity. A report re-run for the same date window is a NEW independent observation, not an update. `etl/xero/reports.py` synthesizes `record_id = f"{report}|{run_id}"` so `staging_xero.report_snapshots`/`report_rows` accumulate history instead of collapsing — this needed `GCSReader._extract_records()` to start passing through the full `.meta.json` dict as `record["meta"]` (additive; other parsers ignore it), since the params/run-id aren't in the payload at all.

Prior art: the deprecated silver layer's `fact_report_row.sqlx` (`docs/deprecated/layeSystem/` era — see `docs/deprecated/README.md`) solved this exact tree-flattening problem in SQL against a bronze JSON envelope; `etl/xero/reports.py` reimplements the same modeling in Python (recursive walk, much more natural than nested `JSON_QUERY_ARRAY`/`UNNEST`), verified against live 2.0 payloads to confirm the shape hasn't changed.
