"""Xero prepayments parser. Bronze currently empty."""
import logging
...
BRONZE_TABLE = "xero_prepayments"
HEADER_TABLE = "prepayments"
...
def parse_header(record):
    ...
    return {
        ...
        "prepayment_id": p.get("PrepaymentID"),
        "prepayment_type": p.get("Type"), "status": p.get("Status"),
        "line_amount_types": p.get("LineAmountTypes"),
        "contact_id": contact.get("ContactID"), "contact_name": contact.get("Name"),
        "prepayment_date": parse_xero_datetime(p.get("Date")),
...
def run(reader, writer, tenant_id=None, limit=None):
    ...
    logger.info("prepayments: %d rows", len(headers))
