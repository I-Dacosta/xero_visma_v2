"""Xero receipts parser. Bronze currently empty."""
import logging
...
BRONZE_TABLE = "xero_receipts"
HEADER_TABLE = "receipts"
...
def parse_header(record):
    ...
    return {
        ...
        "receipt_id": p.get("ReceiptID"),
        "reference": p.get("Reference"), "status": p.get("Status"),
        "line_amount_types": p.get("LineAmountTypes"), "url": p.get("Url"),
        "contact_id": contact.get("ContactID"), "contact_name": contact.get("Name"),
        "user_id": user.get("UserID"), "user_email": user.get("EmailAddress"),
        "receipt_date": parse_xero_datetime(p.get("Date")),
...
def run(reader, writer, tenant_id=None, limit=None):
    ...
    logger.info("receipts: %d rows", len(headers))
